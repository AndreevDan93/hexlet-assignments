package exercise;

import org.junit.jupiter.api.Test;

import java.util.List;
import java.util.Map;

import static org.assertj.core.api.Assertions.assertThat;

// BEGIN
class SorterTest {

    @Test
    void takeOldestMansTest1() {

        List<Map<String, String>> users = List.of(
                Map.of("name", "Vladimir Nikolaev", "birthday", "1990-12-27", "gender", "male"),
                Map.of("name", "Andrey Petrov", "birthday", "1989-11-23", "gender", "male"),
                Map.of("name", "Anna Sidorova", "birthday", "1996-09-09", "gender", "female"),
                Map.of("name", "John Smith", "birthday", "1989-03-11", "gender", "male"),
                Map.of("name", "Vanessa Vulf", "birthday", "1985-11-16", "gender", "female"),
                Map.of("name", "Alice Lucas", "birthday", "1986-01-01", "gender", "female"),
                Map.of("name", "Elsa Oscar", "birthday", "1970-03-10", "gender", "female")
        );

        List<String> result = Sorter.takeOldestMans(users);
        List<String> answerList = List.of("John Smith", "Andrey Petrov", "Vladimir Nikolaev");

        assertThat(result).isEqualTo(answerList);
    }

    @Test
    void takeOldestMansTest2() {

        List<Map<String, String>> users = List.of(
                Map.of("name", "Vladimir Nikolaev", "birthday", "1990-12-27", "gender", "female"),
                Map.of("name", "Andrey Petrov", "birthday", "1989-11-23", "gender", "female"),
                Map.of("name", "Anna Sidorova", "birthday", "1996-09-09", "gender", "female"),
                Map.of("name", "John Smith", "birthday", "1989-03-11", "gender", "female"),
                Map.of("name", "Vanessa Vulf", "birthday", "1985-11-16", "gender", "female"),
                Map.of("name", "Alice Lucas", "birthday", "1986-01-01", "gender", "female"),
                Map.of("name", "Elsa Oscar", "birthday", "1970-03-10", "gender", "female")
        );

        List<String> result = Sorter.takeOldestMans(users);
        List<String> answerList = List.of();

        assertThat(result).isEqualTo(answerList);
    }

    @Test
    void takeOldestMansTest3() {

        List<Map<String, String>> users = List.of(
                Map.of("name", "Vladimir Nikolaev", "birthday", "1997-12-27", "gender", "male"),
                Map.of("name", "Andrey Petrov", "birthday", "1996-11-23", "gender", "male"),
                Map.of("name", "Anna Sidorova", "birthday", "1995-09-09", "gender", "male"),
                Map.of("name", "John Smith", "birthday", "1994-03-11", "gender", "male"),
                Map.of("name", "Vanessa Vulf", "birthday", "1993-11-16", "gender", "male"),
                Map.of("name", "Alice Lucas", "birthday", "1992-01-01", "gender", "male"),
                Map.of("name", "Elsa Oscar", "birthday", "1991-03-10", "gender", "male")
        );

        List<String> result = Sorter.takeOldestMans(users);
        List<String> answerList = List.of("Elsa Oscar", "Alice Lucas", "Vanessa Vulf",
                "John Smith", "Anna Sidorova", "Andrey Petrov", "Vladimir Nikolaev");

        assertThat(result).isEqualTo(answerList);
    }

}
// END


